...
ggplot(courbe_b, aes(x = T, y = value)) + geom_line(aes(color = variable),size=1)+theme_minimal()
head(M.3)
...
#
ggplot(data = M.3, aes(x= T ,y = Lx25)) + geom_line() + geom_point()

# Calcul des probalit�s de mortalit� invalidit� [25 ; 60]
Lx_25 <- M.3$Lx25
m <- length(Lx_25)
p <- matrix(0,m,m); d <- p
for(i in 1:(m-1)){
...
q <- 1-p
apply(d,2,sum)[1:10]
# Calculons la proba 1P1 proba qu'un individu d'age 25 vivve jusqu'� 26ans
p[1,1]
M.3
...
TM_25
#write.csv(data.table(A),	file	=	"Life table TM_25.csv",row.names=FALSE)

#Extraction de 1P1
pxt(TM_25,x=0,t=1) # proba de survie individus d'age 25ans à 26 ans 
qxt(TM_25, x = 1, t=1)
...
# Taux de survie
log(M.3$Lx25)
log_Lx25 <- log(M.3$Lx25)
M.4 <- data.frame(M.3,log_Lx25)
plot(x=T, y=M.3$Lx25, log = "y")
# Echelle en log 
# s'il n'y a que y en log :
y <- exp(1:10)
x <- 1:10 
plot(x,y,log="y",panel.first=abline(lm(log10(y)~x),col="red"),pch=19)
...
##########################################################
 #  Mod�lisation Kaplan Meier ####
##########################################################
# Nous chargons les objets de censure des donn�es de la table de mortalit� invalidit�

Mort_inval_2 <- read.table("C:/Users/Pierre HERIAUT/Desktop/Suite_FilRouge/MortaInval V3bis_2.csv", sep=";", header = TRUE)
d_time <-c(25:64)
censur_mort_inval_2 <- data.frame(Mort_inval_2,d_time)  
library(survival) 
# objet de srurvie est not�e mydata.surv.25
attach(censur_mort_inval_2)
jeu <- data.frame(N_1,d_time) 
detach(censur_mort_inval_2)
donnee_N2 <- Mort_inval_2$N_2
censur_mort_inval_2.surv <- Surv(d_time, donnee_N2)
censur_mort_inval_2.surv
###############
survfit(formula,jeu,conf.int=.95,type=c("kaplan-meier","fleming-harrington"),error=c("greenwood","tsiatis"),conf.type=c("log","log-log","plain","none"))
survfit(censur_mort_inval_2.surv~1)
plot(survfit(censur_mort_inval_2.surv~1), data=jeu,col=c("black","red"),main="Estimateur de kaplan meier de la fonction de survie")

w=survfit(censur_mort_inval_2.surv~1,data=jeu,type="kaplan-meier")
#plot(w, data=jeu,col=c("black","red"),main="Courbe obtenue par kaplan meier")
